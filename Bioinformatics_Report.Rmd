...
ggplot(aes(x = Genus,
           y = Abundance)) +
  geom_boxplot() +
  ggtitle("Most abundant genera in refrigerated treatment group")
```


```{r genera-stats}
# run a statistical test for genera abundance in refrigerated treatment group
melted_phyloseq %>%
  filter(type == "Refrigerated") %>%
  group_by("Genus") %>%
  filter(Abundance > 250) %>%
  wilcox.test(data = .,
              as.numeric(Abundance) ~ Genus) %>%
  broom::glance(x = .) %>%
  knitr::kable()
```
